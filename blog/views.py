...
def post_list(request):
    ...
    klines = client.get_historical_klines("BTCUSDT", Client.KLINE_INTERVAL_1DAY, "6 month ago UTC")
    unix_timestamps = []
    for kline in klines:
    ...
    for unix_timestamp in unix_timestamps:
        timestamps.append(datetime.fromtimestamp(unix_timestamp / 1000).strftime("%m/%d/%Y, %H:%M:%S"))
